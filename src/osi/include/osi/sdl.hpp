...
{
    // NOTE: Might not be necessary, use Config instead?
    ~SDLConfig() = default;
};
 ...
 * @param sdl_cfg Configuration structure containing properties of the initialized window
...
/**
 * Sets pointers for `keyboard`, `mouse`, and `window` used in the SDL loop.
 * @param keyboard Keyboard owned by the `Application` class
 * @param mouse Mouse owned by the `Application` class
 * @param window Window owned by the `Application` class
 */
